...
v-form(ref="form" :model="entity")
  ...
  div(v-for="field in template.fields")
    v-dynamic-field(:label="field.name" :type="field.type" @input="field.sluggable ? updateSlug($event) : null" v-model="entity[field.slug]")
    v-text-field(v-if="field.sluggable" label="Slug" :prefix="templateUrl" v-model="entity.slug" :error-messages="entity.errors.slug" hint="The URL path of your content (e.g. my-first-post)")
